...
const MyHistory = () => {
  ...
  const getAllBookings = async () => {
    try {
      ...
      const res = await fetch(
        `https://rgtours.onrender.com/api/booking/get-allUserBookings/${currentUser?._id}?searchTerm=${search}`,
        { credentials: "include" }
      );
  ...
  const handleHistoryDelete = async (id) => {
    try {
      setLoading(true);
      const res = await fetch(
        `https://rgtours.onrender.com/api/booking/delete-booking-history/${id}/${currentUser._id}`,
        {
          method: "DELETE",
          credentials: "include",
